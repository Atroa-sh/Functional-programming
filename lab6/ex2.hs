...
--safeTail nie przyjmuje argumentu typu Just x
...
-- w ostatnim wyrzuca blac bo wywolujemy extractMaybe od Nothing
-- (>^$>) = extract (^) and apply ($)
(>^$>) :: Maybe a -> (a -> Maybe b) -> Maybe b
Nothing  >^$> _ = Nothing
(Just x) >^$> f = f x
infixl 1 >^$>
-- u mnie blad nadal sie pojawia
...
--personalnie 1 albo 3 sa najbardziej czytelne
...
--nie jestem pewnien czy o to chodzi
